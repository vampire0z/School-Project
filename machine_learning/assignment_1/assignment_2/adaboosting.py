...
def normalise():
	...
	PCA_train = pca.fit_transform(scale(X_train))
	PCA_test = pca.fit_transform(scale(X_test))


	# run random forest
	adaboosting(PCA_train,y_train,PCA_test,y_test)

def adaboosting(train_data, train_label, test_data, test_label):
	print("Adaboosting Classifer")
	clf = AdaBoostClassifier(n_estimators=100)
	clf.fit(train_data, train_label)

	print("Adaboosting Classifier Predict:")
	y_true = test_label
